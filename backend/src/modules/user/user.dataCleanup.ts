...
export async function deleteProjectsByIds(db: Db, projectIds: string[]) {
    ...
    const keys = await captureInlineDocumentCleanup(db, { projectIds: ownedProjectIds });
    // Parent DELETE owns the relational cascades and all durable cleanup.
...
export async function deleteUserAccountData(
    ...
    // With workers disabled, preserve an inline cleanup snapshot. Normal
    // deployments rely on the version trigger's transactional cleanup jobs.
    const doomedVersionPaths = await captureInlineDocumentCleanup(db, { documentIds });
    ...
    await completeInlineDocumentCleanup(db, doomedVersionPaths);
