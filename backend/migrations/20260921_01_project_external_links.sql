...
-- Persist the Zoho Deal id and SharePoint folder URL on projects so the
-- Matters list and matter page can link out. Safe to re-run. RLS is
-- unchanged: the new columns inherit the existing projects policies.
...
revoke all on function public.create_project_with_memory(
  uuid, text, text, text, uuid, boolean, text, text, text, text
)
